...
export default function StrategicReport({ transactions, selectedMonth }: { transactions: Transaction[], selectedMonth: string }) {
  ...
  // Historical trend (simplified for prototype)
  ...
  // ── Cashflow Chart Data (30-day index) ──────────────────────────────────────
  const lineData = useMemo(() => {
    ...
    for (let i = 1; i <= days; i++) {
      ...
      data.push({ day: i, income: dayIncome, expense: dayExpense });
    }
    return data;
  ...
  const donutData = useMemo(() => {
    ...
    return [
      { name: 'Fixed Needs', value: needs, color: '#10b981' }, // emerald-500
      { name: 'Variable Wants', value: wants, color: '#f43f5e' }, // rose-500
      { name: 'Savings/Invest', value: savings, color: '#3b82f6' }, // blue-500
    ].filter(d => d.value > 0);
  ...
  const microLeaks = useMemo(() => {
    ...
    return Object.entries(freq)
      .filter(([_, d]) => d.count >= 4) // Common leaks happen frequently
      .sort((a,b) => b[1].count - a[1].count)
      .slice(0, 5);
  }, [transactions, selectedMonth]);

  return (
    <div className="bg-zinc-950 text-slate-200 p-4 md:p-8 min-h-screen font-sans selection:bg-emerald-500/30">
        ...
        <div>
          ...
          <p className="text-slate-400 font-medium tracking-wide flex items-center gap-2">
            <ShieldCheck size={16} className="text-emerald-500" />
             Aset Terkelola & Analisis Strategi {monthName}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <ExecutiveCard 
            ...
            value={fmt(150000000)} // Mock for UI spec
        ...
        <ExecutiveCard 
            ...
            trend={savingRate >= 20 ? "Target Achieved" : "Below Optimal"}
            icon={<Target size={24} />}
        ...
        <ExecutiveCard 
            ...
            value="4.5 Months"
            trend="Cash reserve depletion rate"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-10">
        ...
        <div className="bg-zinc-900/50 border border-slate-800 rounded-2xl p-6">
          <h2 className="text-lg font-bold text-white mb-2">Structural Rigidity</h2>
          <p className="text-slate-500 text-xs mb-6">Efisiensi alokasi modal bulan ini.</p>
      ...
      {/* 4. The "Micro-Leak" Radar & Insights */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
        <div className="bg-zinc-900/50 border border-slate-800 rounded-2xl p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
               <AlertTriangle size={18} className="text-amber-500" /> High-Frequency Radar
            </h2>
             ...
             {microLeaks.length > 0 ? microLeaks.map(([name, data]) => (
                 <div key={name} className="flex items-center justify-between p-3 bg-zinc-950/50 rounded-xl border border-slate-800/50">
                     ...
                     <div className="text-right">
                         ...
                         <div className="text-[10px] text-slate-600">Leak Potential: High</div>
                     </div>
                 </div>
             )) : (
                 <div className="text-center py-10 text-slate-500 italic text-sm">No high-frequency leaks detected. Strategic discipline is intact.</div>
             )}
          </div>
        </div>
        ...
        <div className="relative group p-1 rounded-3xl overflow-hidden">
            ...
            <div className="relative h-full bg-zinc-900 border border-slate-800 rounded-2xl p-8 shadow-2xl">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-lg font-black text-white tracking-widest uppercase">AI Monthly Verdict</h2>
                </div>
                <div className="prose prose-invert prose-sm">
                    <p className="text-slate-300 leading-relaxed italic text-lg font-medium">
                        "{expenseChange > 10 ? "Cashflow is under pressure." : "Strategic structural solidity is high."} Expense in '{donutData[1]?.name || 'Variable Wants'}' has shifted {Math.abs(expenseChange).toFixed(1)}% compared to last month. 
                        Recommendation: Move {fmt(Math.max(surplus/2, 500000))} to your Sinking Funds immediately to sterilize liquid excess and prevent lifestyle creep."
                    </p>
                </div>
                <div className="mt-8 pt-6 border-t border-slate-800/50 flex justify-between items-center text-[10px] font-black tracking-widest text-slate-600 uppercase">
                    <span>Status: Highly Analytical</span>
      ...
      <div className="bg-zinc-900/50 border border-slate-800 rounded-2xl p-6">
        <h2 className="text-lg font-black text-white mb-8 flex items-center gap-2">
            <Target size={20} className="text-blue-500" /> Wealth Strategic Goals
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <GoalProgress title="Emergency Fund" current={40} color="emerald" />
            <GoalProgress title="Vehicle Downpayment" current={15} color="blue" />
            <GoalProgress title="Annual Taxes" current={100} color="amber" isCompleted />
        </div>
      </div>
    </div>
  );
...
function GoalProgress({ title, current, color, isCompleted }: { title: string, current: number, color: 'emerald'|'blue'|'amber', isCompleted?: boolean }) {
    ...
    return (
        <div className="space-y-3">
            ...
            {isCompleted && <div className="text-[9px] font-black text-emerald-500 uppercase flex items-center gap-1"><ShieldCheck size={10}/> Strategic Target Fully Met</div>}
        </div>
    )
}
